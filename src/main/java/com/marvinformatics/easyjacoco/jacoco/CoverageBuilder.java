...
/**
 * Builder for hierarchical {@link ICoverageNode} structures from single {@link IClassCoverage}
 * nodes. The nodes are feed into the builder through its {@link ICoverageVisitor} interface.
 * Afterwards the aggregated data can be obtained with {@link #getClasses()}, {@link
 * #getSourceFiles()} or {@link #getBundle(String)} in the following hierarchy:
 *
 * <pre>
...
public class CoverageBuilder implements ICoverageVisitor {
  ...
  private void build() {
    ...
    this.sourcefiles = new HashMap<String, ISourceFileCoverage>();
    for (final IClassCoverage c : classes.values()) {
      for (final SourceNodeImpl fragment : ((ClassCoverageImpl) c).getFragments()) {
        final SourceNodeImpl classCoverage = (SourceNodeImpl) classes.get(fragment.getName());
  ...
  public void visitCoverage(final IClassCoverage coverage) {
    ...
    if (dup != null) {
      if (dup.getId() != coverage.getId()) {
        log.warn("Can't add different class with same name: " + name);
      }
    }
  }
